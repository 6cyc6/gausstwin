...
from gausstwin.cfg.domain import Rope, Gaussians, Particles, Ground, SegPrompt
...
from gausstwin.utils.rope_utils import extract_rope_centerline_from_gaussians
from gausstwin.utils.init_utils import compute_bounding_box, fill_bounding_box_with_spheres, is_point_in_mask
from gausstwin.utils.gs_utils import ransac_plane, down_sample_pcl, create_rigid_object_splats_with_optimizers, vis_render_gs_cam


class RopeBuilder:
    def __init__(self, cfg: RopeConfig, device: str = "cuda:0"):
        self.cfg = cfg
    ...
    def build(
        ...
        # ================== Step 3: Get the ground plane function (table) ====================== #
        self.fit_ground_plane()
        
        # ==================== Step 4: Extract points within the bounding box =================== #
        self.extract_spheres()
        
        # ================== Step 5: Run gs optimization to get particles ======================= #
        self.build_gaussians()
        
        # =========================== Step 6: Extract the 3D skeleton =========================== #
        self.extract_central_line()
    ...
    def extract_spheres(self):
        ...
        self.obj_radius = {}
        self.obj_gp_pts_mean_dict = {}
        self.obj_gp_pts_dict = {}
        self.obj_gp_labels_dict = {}
        for obj in self.cfg.obj_list:
            ...
            # polyscope_vis_pcl([pts_obj.cpu().numpy()])
            ...
            # save the pts and labels for GPIS
            # sample points in a larger box
            length = max_coord - min_coord
            min_coord = min_coord - length * 0.15
            max_coord = max_coord + length * 0.15
            sample_pts = fill_bounding_box_with_spheres(min_coord, max_coord, self.cfg.particle_radius - 0.001)
            sample_pts = sample_pts.to(self.device)
            # get points and labels
            pts_inliers, mask_obj_sample =  self.get_inlier_pts(sample_pts, self.obj_mask_dict[obj], return_mask=True)
            mask_inliers = mask_obj_sample[mask_obj_sample == 1]  # mask for inliers
            pts_outliers = sample_pts[mask_obj_sample == 0]  # points that are not in the object mask
            mask_outliers = mask_obj_sample[mask_obj_sample == 0]  # mask for outliers
            n_inliers = pts_inliers.shape[0]
            num_pick = torch.min(torch.tensor([pts_outliers.shape[0], n_inliers])) # down sample outliers
            indices = torch.randperm(pts_outliers.shape[0])[:num_pick]
            pts_outliers = pts_outliers[indices]
            mask_outliers = mask_outliers[indices]
            mask_obj_sample = torch.cat([mask_inliers, mask_outliers], dim=0)  # (N, )
            sample_pts = torch.cat([pts_inliers, pts_outliers], dim=0)  # (N, 3)
            
            self.obj_gp_pts_mean_dict[obj] = torch.mean(sample_pts, dim=0)
            self.obj_gp_pts_dict[obj] = sample_pts - self.obj_gp_pts_mean_dict[obj]
            self.obj_gp_labels_dict[obj] = -(mask_obj_sample * 2 - 1) # -1 for inside, 1 for outside
            
            if self.cfg.vis_init_particles:
                polyscope_vis_pcl([self.obj_sphere_centers[obj].cpu().numpy(), pts_obj.cpu().numpy()])
        logging.info(f"Sphere centers are extracted info is loaded.")
    ...
    # ========================================================================================== #
    def extract_central_line(self):
        """
        Extract the 3D centerline of the rope by rendering a top-down view of the
        trained Gaussians, skeletonizing the binary mask, and back-projecting the
        resampled 2D centerline points to 3D.
        """
        if self.cfg.remove_ends:
            n_segments = self.cfg.n_segments + 2
        else:
            n_segments = self.cfg.n_segments
            
        particles_3d, particle_radius, is_loop = extract_rope_centerline_from_gaussians(
            self.splats_gaussians,
            n_segments=n_segments,
            table_h=-self.plane_h.item(), 
            obj_sphere_centers=self.obj_sphere_centers["Rope"],
            device=self.device, 
            vis=self.cfg.vis_init_particles
        )

        logging.info(f"Extracted centerline: {len(particles_3d)} points, particle_radius={particle_radius}, is_loop={is_loop}")

        # store results
        if self.cfg.remove_ends:
            self.obj_particles_dict = {"Rope": particles_3d[1:-1]} # remove the two endpoints for better optimization stability; (N, 3)
        else:
            self.obj_particles_dict = {"Rope": particles_3d}
        self.rope_radius = particle_radius

        # compute spacing for logging
        diffs = particles_3d[2:] - particles_3d[1:-1]
        spacing = torch.norm(diffs, dim=1)
        logging.info(
            f"Rope centerline: {len(particles_3d)} particles, "
            f"spacing={spacing.mean().item():.4f}m (std={spacing.std().item():.4f})"
        )
    ...
    def assign_gaussians_to_particles(self):
        # assign the gaussians to the objects
        pts_gaussians = self.splats_gaussians["means"].detach().clone()
        centers = torch.concat([self.obj_sphere_means[obj].unsqueeze(0) for obj in self.cfg.obj_list], dim=0) # center of particles
        _, assignments = knn(centers, pts_gaussians, k=1, num_workers=10) 
        
        # save gaussians for each object
        ...
        for i, obj in enumerate(self.cfg.obj_list):
            obj_indices = torch.where(assignments == i)[0]
            self.obj_gaussians_means_dict[obj] = self.splats_gaussians["means"][obj_indices].detach()
            self.obj_gaussians_quats_dict[obj] = F.normalize(self.splats_gaussians["quats"][obj_indices]).detach()
            self.obj_gaussians_colors_dict[obj] = torch.sigmoid(self.splats_gaussians["colors"][obj_indices]).detach()
            self.obj_gaussians_opacities_dict[obj] = torch.sigmoid(self.splats_gaussians["opacities"][obj_indices]).detach()
            self.obj_gaussians_scales_dict[obj] = torch.exp(self.splats_gaussians["scales"][obj_indices]).detach()
        
        self.obj_gaussians_assign_idx_dict = {}
        # assign the gaussians to the particles
        for i, obj in enumerate(self.cfg.obj_list):
            pts_obj_gaussians = self.obj_gaussians_means_dict[obj]
            pts_obj_particles = self.obj_particles_dict[obj]
            # Finds for each element in pts_obj_gaussians the 1 nearest point in pts_obj_particles
            _, assignments = knn(pts_obj_particles, pts_obj_gaussians, k=1, num_workers=10)
            self.obj_gaussians_assign_idx_dict[obj] = assignments.to(dtype=torch.int16)

            logging.info(f"Object: {obj}")
            logging.info(f"Number of Gaussians: {self.obj_gaussians_means_dict[obj].shape[0]}")
            logging.info(f"Number of Particles: {pts_obj_particles.shape[0]}")
    ...
    def convert_to_body_frame(self):
        ...
        for i, obj in enumerate(self.cfg.obj_list):
            if obj == "Rope":
                # For rope, create individual poses for each segment
                particles_means = self.obj_particles_dict[obj].cpu().numpy()
                n_particles = len(particles_means)
                
                # Create pose matrix for each particle/segment
                poses = []
                poses_save = []
                for j in range(n_particles):
                    mat_seg = np.eye(4, dtype=np.float32)
                    
                    # Set position as particle position
                    mat_seg[:3, 3] = particles_means[j]
                    
                    # Calculate z_axis (direction to next particle)
                    if j < n_particles - 1:
                        # Direction to next particle
                        z_dir = particles_means[j + 1] - particles_means[j]
                    else:
                        # For last particle, use same direction as second-to-last
                        if n_particles > 1:
                            z_dir = particles_means[j] - particles_means[j - 1]
                        else:
                            # Single particle case, use default direction
                            z_dir = np.array([1.0, 0.0, 0.0])
                    
                    # Normalize z_axis
                    z_axis_norm = np.linalg.norm(z_dir)
                    if z_axis_norm > 1e-6:
                        z_axis = z_dir / z_axis_norm
                    else:
                    ...
                    # y_axis is cross product of z and x
                    y_axis = np.cross(z_axis, x_axis)
                    y_axis_norm = np.linalg.norm(y_axis)
                    if y_axis_norm > 1e-6:
                        y_axis = y_axis / y_axis_norm
                    else:
                        # If z_axis is parallel to x_axis, use a different reference
                    ...
                    poses.append(mat_seg)
                    poses_save.append(mat_seg.tolist())
                
                # Store all poses for rope segments
                self.obj_particles_pose_dict[obj] = poses_save
                
                # Transform Gaussians based on their assigned particle indices
                gaussians_means = self.obj_gaussians_means_dict[obj]
                gaussians_quats = self.obj_gaussians_quats_dict[obj]
                gaussian_assign_idx = self.obj_gaussians_assign_idx_dict[obj]
                
                # For rope, keep particles in world coordinates but transform Gaussians
                # Transform Gaussians to their respective local particle frames
                gaussians_means_local = torch.zeros_like(gaussians_means)
                gaussians_quats_local = torch.zeros_like(gaussians_quats)
                
                for j in range(n_particles):
                    # Find Gaussians assigned to this particle
                    gaussian_mask = (gaussian_assign_idx == j)
                    if gaussian_mask.sum() > 0:
                        # Get transformation matrix for this particle
                        mat_inv = np.linalg.inv(poses[j])
                        mat_inv_torch = torch.tensor(mat_inv, device=self.device)
                        
                        # Transform Gaussian positions
                        gaussians_world = gaussians_means[gaussian_mask]
                        gaussians_local = (mat_inv_torch[:3, :3] @ gaussians_world.T + mat_inv_torch[:3, 3:4]).T
                        gaussians_means_local[gaussian_mask] = gaussians_local
                        
                        # Transform Gaussian quaternions
                        quat_gaussians = gaussians_quats[gaussian_mask].cpu().numpy()
                        quat_mat = R.from_quat(quat_gaussians, scalar_first=True).as_matrix()  # type:ignore # (N, 3, 3)
                        quat_mat_local = mat_inv[:3, :3] @ quat_mat
                        quat_local = R.from_matrix(quat_mat_local).as_quat(scalar_first=True) # type:ignore # (N, 4)
                        gaussians_quats_local[gaussian_mask] = torch.from_numpy(quat_local).to(self.device).type_as(gaussians_quats)
                
                # Update Gaussian positions and orientations
                self.obj_gaussians_means_dict[obj] = gaussians_means_local
                self.obj_gaussians_quats_dict[obj] = gaussians_quats_local
                ...
                # particles_sdf_dir = self.obj_particles_sdf_dir_dict[obj]
                gaussians_means = self.obj_gaussians_means_dict[obj]
                gaussians_quats = self.obj_gaussians_quats_dict[obj]
                ...
                # particles_sdf_dir = (mat_obj_inv_torch[:3, :3] @ particles_sdf_dir.T).T
                ...
                # self.obj_particles_sdf_dir_dict[obj] = particles_sdf_dir
    ...
    # ========================================================================================== #
    def save_objects_and_ground(self):
        """
        Save the objects and ground.
        """
        # save the rope
        for i, obj in enumerate(self.cfg.obj_list):
            # save the particles
            particles = Particles(
                ...
                radii=[self.rope_radius] * self.obj_particles_dict[obj].shape[0],
                sdf_dir=[], 
                sdf_mag=[], 
                sdf_is_boundary=[], 
            )
            ...
            rope = Rope(
                ...
                particles=particles,
                gaussians=gaussians,
                poses=self.obj_particles_pose_dict[obj], 
                gs_particle_indices=self.obj_gaussians_assign_idx_dict[obj].tolist(),
                prompts=prompts,
            )
            rope.save(exp_name=self.cfg.exp_name)
            logging.info(f"Rope is saved.")


        if not self.cfg.save_ground_gaussians:
            self.ground_gaussians = None
    ...
    def clean_point_cloud(self, pts, n_pts=30000):
        ...
        pcl.points = o3d.utility.Vector3dVector(pts.cpu().numpy())
        pcl_1, _ = pcl.remove_radius_outlier(nb_points=35, radius=0.03)
        pcl_cleaned, _ = pcl_1.remove_statistical_outlier(nb_neighbors=20, std_ratio=2.0)

        return torch.tensor(np.asarray(pcl_cleaned.points), device=self.device, dtype=torch.float32)
    
    
    def clean_obj_point_cloud(self, pts, pts_rgb, n_pts=30000, ground_h=0.0):
        """
        Clean the point cloud by removing outliers.
        """
        # remove points that are below the ground height
        indices = pts[:, 2] >= ground_h
        pts = pts[indices]  # (N, 3)
        pts_rgb = pts_rgb[indices]  # (N, 3)
        ...
        if N <= n_pts:
            ...
            indices = torch.randperm(N, device=pts.device)[:n_pts]
        pts = pts[indices]  # (n_pts, 3)
        pts_rgb = pts_rgb[indices]  # (n_pts, 3)

        # clean the point cloud using Open3D
        pcl = o3d.geometry.PointCloud()
        pcl.points = o3d.utility.Vector3dVector(pts.cpu().numpy())
        pcl_1, indices = pcl.remove_radius_outlier(nb_points=35, radius=0.03)
        pts_rgb = pts_rgb[indices]  # (n_pts, 3)
        
        pcl_cleaned, indices = pcl_1.remove_statistical_outlier(nb_neighbors=20, std_ratio=2.0)
        pts_rgb = pts_rgb[indices]  # (n_pts, 3)

        return torch.tensor(np.asarray(pcl_cleaned.points), device=self.device, dtype=torch.float32), pts_rgb
    ...
    def get_inlier_pts(self, pts, masks, return_mask=False, threshold=0.008):
        ...
        for cam_idx in range(self.cam.Ks.shape[0]):
            # check for sphere centers
            us, vs = self.cam.world_to_image(pts, cam_idx) # flip of the mask is considered
            ...
            depth = self.depths[cam_idx]
            depth_value = depth[vs, us].squeeze(-1)  # get the depth value
