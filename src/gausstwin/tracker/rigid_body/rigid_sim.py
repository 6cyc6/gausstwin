...
from gausstwin.gaussians.visual_forces import VisualForcesRigidBody, warp_batch_transform, apply_transform_means_kernel
from gausstwin.utils.warp_utils import transform_rigid_body_gaussians_kernel, set_visual_body_force_new_kernel, apply_body_force_kernel, velocity_damping_rigid_body_kernel


class RigidMultiSimulator(Simulator):
    def __init__(
        self, 
        builder: RigidBuilder, 
        ...
        # visual forces
        self.visual_forces = VisualForcesRigidBody(
            gaussian_model=self.gaussian_model, 
            gaussian_state=self.gaussian_state, 
            bodies_in_track=builder.body_in_track_idx_list,
            mode=cfg.mode,
        )
        self.appearance_optimizer = AppearanceOptimizer(self.gaussian_state)
        self.kp_gains = torch.tensor([[cfg.gain_1], [cfg.gain_2]], dtype=torch.float32, device=self.device)  # shape: (2,)
        
        # save indices
        self.body_in_track_idx_list = builder.body_in_track_idx_list
        self.body_in_track_idx_local_list = builder.body_in_track_idx_local_list
        self.body_in_track_idx_wp = wp.array(self.body_in_track_idx_list, dtype=wp.int32, device=self.device)
    ...
    def simulate(self):
        # Assign forces
        wp.launch(
            kernel=apply_body_force_kernel,
            dim=len(self.body_in_track_idx_list),
            inputs=[
                self.visual_forces.forces_bodies, 
                self.visual_forces.moments_bodies,
                self.body_in_track_idx_wp,
            ],
    ...
    def step(
        ...
        if clear_forces:
            self.visual_forces.forces_bodies.zero_()
            self.visual_forces.moments_bodies.zero_()
    ...
    def update_robot_state(self, positions: torch.tensor, quats: torch.tensor, linear_vel: torch.tensor = None, angular_vel: torch.tensor = None):
        ...
        body_q_0 = wp.to_torch(self.state_0.body_q) # type: ignore
        ...
        if linear_vel is not None and angular_vel is not None:
            body_qd_0 = wp.to_torch(self.state_0.body_qd) # type: ignore
    ...
    def transform_gs(self):
        # transform the 3D Gaussian States of objects and Robots
        wp.launch(
            kernel=transform_rigid_body_gaussians_kernel, 
            dim=self.gaussian_model.num_gaussians, 
            inputs=[
                self.gaussian_model.means, self.gaussian_model.quats, 
                self.gaussian_model.body_ids,
                self.state_0.body_q,
                self.ground_h,
            ], 
    ...
    def compute_visual_forces(self, gt_pixels_list, gt_masks_list, n_iter=None):
        ...
        with torch.no_grad():
            ...
            if self.cfg.mode == 2 or self.cfg.mode == 3 or self.cfg.mode == 4:
                ...
                # get the object mean positions
                body_t = wp.to_torch(wp.clone(self.state_0.body_q))[self.body_in_track_idx_list, :3] 
                self.visual_forces.tracked_translations = body_t
        ...
        for step in range(max_iter):
            ...
            trans_means, trans_quats = warp_batch_transform(
                ...
                self.gaussian_model.rigid_indices, self.gaussian_model.rigid_body_local_ids,
                self.gaussian_model.num_obj_gaussians, 
            )
        ...
        wp.launch(
            ...
            dim=self.gaussian_model.num_obj_gaussians, 
            inputs=[
                target_means,
        ...
        prev_means = self.gaussian_state.means.detach()[self.gaussian_model.rigid_indices] 
        ...
        with torch.no_grad():
            ...
            wp.launch(
                kernel=set_visual_body_force_new_kernel,
                dim=self.gaussian_model.num_obj_gaussians,
                inputs=[
                    self.cfg.kp_f, 
                    self.cfg.kp_m,
                    prev_means,
                    target_means[self.gaussian_model.rigid_indices], 
                    self.gaussian_state.opacities.detach()[self.gaussian_model.rigid_indices], 
                    self.gaussian_particle_id_weights_wp,
                    self.gaussian_model.rigid_body_ids, 
                    self.gaussian_model.rigid_body_local_ids, 
                    self.state_0.body_q,
                ], 
                outputs=[self.visual_forces.forces_bodies, self.visual_forces.moments_bodies],
            )

            if len(self.body_in_track_idx_list) > 1:
                self.visual_forces.forces_bodies *= self.kp_gains
                self.visual_forces.moments_bodies *= self.kp_gains

        # logging.info(f"After Transform, Time eplased: {time.time() - t_s:.4f}")
